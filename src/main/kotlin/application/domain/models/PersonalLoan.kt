...
class PersonalLoan: LoanRule {
    private companion object {
        const val MAX_LOAN_PERCENTAGE = 0.25
        const val PROPOSAL_VALIDITY_DAYS = 7
    }

    override val loanType: LoanType
        get() = LoanType.PERSONAL

    override fun calculateTax(income: Income): Tax {
        val taxRateValue = when {
            income.value <= BigDecimal(2000) -> BigDecimal(1.5)
            income.value <= BigDecimal(5000) -> BigDecimal(1.3)
            income.value <= BigDecimal(10000) -> BigDecimal(1.1)
            else -> BigDecimal(0.9)
        }
