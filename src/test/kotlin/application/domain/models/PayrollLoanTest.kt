...
class PayrollLoanTest {

    private val loan = PayrollLoan()

    @Test
    fun `taxa deve ser 1,4 para renda ate 2000`() {
        val tax = loan.calculateTax(Income.from(2000.0))
        assertEquals(0, BigDecimal(1.4).compareTo(tax.value))
    }
    ...
    @Test
    fun `taxa deve ser 1,2 para renda entre 5001 e 10000`() {
    ...
    @Test
    fun `valor maximo deve ser 30 porcento da renda`() {
        val income = Income.from(10000.0)
        val amount = loan.calculateMaxLoanAmount(income)
        val expected = income.value.multiply(BigDecimal(0.3))
    ...
    @Test
    fun `loanType deve ser PAYROLL`() {
        assertEquals(LoanType.PAYROLL, loan.loanType)
    }

    @Test
    fun `nao deve estar disponivel para SELF_EMPLOYED`() {
        assertFalse(loan.availableTo().contains(Profile.SELF_EMPLOYED))
    }
